...
class Agent:
    ...
    def __init__(self, agent_id: int, x: int, y: int, tile: Tile, profession: Profession = Profession.GATHERER):
        ...
        # 交易相关属性
        self.wealth = 0.0                    # 财富（简化用健康值代表）
        self.trades_made = 0                 # 完成的交易次数
        self.trades_successful = 0          # 成功交易次数
        self.preferred_items = []            # 偏好交易的物品类型
    ...
    def integrate_with_economy(self, economy):
        """与经济系统集成，设定交易偏好"""
        self.economy = economy
        
        # 延迟导入经济系统相关类以避免循环导入
        from .economy import ItemType
        
        # 根据职业设定交易偏好
        if self.profession == Profession.HUNTER:
            self.preferred_items = [ItemType.FOOD, ItemType.WEAPON]
        elif self.profession == Profession.FARMER:
            self.preferred_items = [ItemType.FOOD, ItemType.TOOL]
        elif self.profession == Profession.CRAFTSMAN:
            self.preferred_items = [ItemType.TOOL, ItemType.WEAPON, ItemType.CLOTHING]
        elif self.profession == Profession.GATHERER:
            self.preferred_items = [ItemType.FOOD, ResourceType.WOOD, ItemType.WATER]
    
    def has_resources_for_trading(self, item_type: ItemType, quantity: int) -> bool:
        """检查是否有足够资源用于交易"""
        if item_type == ItemType.FOOD:
            return self.inventory.get(ResourceType.FOOD, 0) >= quantity
        elif item_type == ItemType.WATER:
            return self.inventory.get(ResourceType.WATER, 0) >= quantity
        elif item_type == ItemType.WOOD:
            return self.inventory.get(ResourceType.WOOD, 0) >= quantity
        elif item_type == ItemType.TOOL:
            return self.inventory.get(ResourceType.MINERAL, 0) >= quantity
        return False
    
    def get_trading_capacity(self) -> int:
        """获取交易容量（根据背包剩余空间）"""
        current_total = sum(self.inventory.values())
        return max(0, self.max_inventory - current_total)
    
    def evaluate_trade_opportunity(self, item_type: ItemType, quantity: int, price: float) -> Tuple[bool, str]:
        """评估交易机会是否值得"""
        # 获取市场价格
        ...
        # 个人需求评估
        if item_type == ItemType.FOOD and self.hunger > 70:
            return True, f"急需食物，饥饿度: {self.hunger:.1f}"
        elif item_type == ItemType.WATER and self.thirst > 70:
            return True, f"急需饮水，口渴度: {self.thirst:.1f}"
        elif item_type == ItemType.MEDICINE and self.health < 50:
            return True, f"急需药物，健康度: {self.health:.1f}"
        
        # 资源过剩时愿意出售
        if item_type == ItemType.FOOD and self.inventory.get(ResourceType.FOOD, 0) > 10:
            return True, f"食物过剩，可出售: {self.inventory[ResourceType.FOOD]}"
        elif item_type == ItemType.WATER and self.inventory.get(ResourceType.WATER, 0) > 10:
            return True, f"水过剩，可出售: {self.inventory[ResourceType.WOOD]}"
        
        return False, "当前不需要此物品"
    
    def execute_trade_with(self, other_agent, item_type: ItemType, quantity: int, price: float, is_buying: bool = True) -> Tuple[bool, str]:
        """与另一个Agent执行交易"""
        if is_buying:
            # 买入逻辑
            if not self.has_resources_for_trading(item_type, quantity):
                return False, "买入方资源不足"
            
            # 检查财富（简化用健康值代表）
            cost = price * quantity
            if self.health < cost * 10:  # 简化：健康值代表财富
                return False, "买入方财富不足"
            
            # 执行交易
            # 卖家获得资源
            if item_type == ItemType.FOOD:
                other_agent.inventory[ResourceType.FOOD] = other_agent.inventory.get(ResourceType.FOOD, 0) + quantity
            elif item_type == ItemType.WATER:
                other_agent.inventory[ResourceType.WATER] = other_agent.inventory.get(ResourceType.WATER, 0) + quantity
            elif item_type == ItemType.WOOD:
                other_agent.inventory[ResourceType.WOOD] = other_agent.inventory.get(ResourceType.WOOD, 0) + quantity
            
            # 买家消耗财富，获得物品
            self.health -= cost * 10
            if item_type == ItemType.FOOD:
                self.inventory[ResourceType.FOOD] = self.inventory.get(ResourceType.FOOD, 0) + quantity
            elif item_type == ItemType.WATER:
                self.inventory[ResourceType.WOOD] = self.inventory.get(ResourceType.WOOD, 0) + quantity
            elif item_type == ItemType.WOOD:
                self.inventory[ResourceType.WOOD] = self.inventory.get(ResourceType.WOOD, 0) + quantity
            ...
            other_agent.trades_made += 1
            other_agent.trades_successful += 1
            
            # 记录交易伙伴
            if other_agent.id not in self.trade_partners:
                self.trade_partners.append(other_agent.id)
            if self.id not in other_agent.trade_partners:
                other_agent.trade_partners.append(self.id)
            
            return True, f"成功买入 {quantity} 个 {item_type.value}"
        else:
            # 卖出逻辑
            if not other_agent.has_resources_for_trading(item_type, quantity):
                return False, "卖出方资源不足"
            
            # 检查买家财富
            cost = price * quantity
            if other_agent.health < cost * 10:
                return False, "买家财富不足"
            
            # 执行交易
            # 卖家获得财富，失去资源
            self.health += cost * 10 * 0.9  # 卖家获得90%的价值
            if item_type == ItemType.FOOD:
                self.inventory[ResourceType.FOOD] = max(0, self.inventory.get(ResourceType.FOOD, 0) - quantity)
            elif item_type == ItemType.WATER:
                self.inventory[ResourceType.WOOD] = max(0, self.inventory.get(ResourceType.WOOD, 0) - quantity)
            elif item_type == ItemType.WOOD:
                self.inventory[ResourceType.WOOD] = max(0, self.inventory.get(ResourceType.WOOD, 0) - quantity)
            
            # 买家消耗财富，获得物品
            other_agent.health -= cost * 10
            if item_type == ItemType.FOOD:
                other_agent.inventory[ResourceType.FOOD] = other_agent.inventory.get(ResourceType.FOOD, 0) + quantity
            elif item_type == ItemType.WATER:
                other_agent.inventory[ResourceType.WOOD] = other_agent.inventory.get(ResourceType.WOOD, 0) + quantity
            ...
            return True, f"成功卖出 {quantity} 个 {item_type.value}"
    
    def find_best_trade_partner(self, agents: List['Agent']) -> Optional['Agent']:
        """寻找最佳交易伙伴"""
        best_partner = None
        best_score = -1
        
        for other in agents:
            if other.id != self.id and other.id not in self.trade_partners:
                # 计算交易潜力得分
                score = 0
                
                # 检查互补性
                self_total = sum(self.inventory.values())
                other_total = sum(other.inventory.values())
                
                if abs(self_total - other_total) < 5:  # 资源量相近
                    score += 1
                ...
                # 检查距离（简化：随机模拟距离）
                if random.random() < 0.5:  # 假设50%概率在合理距离内
                    score += 1
    ...
    def make_automatic_trades(self, agents: List['Agent'], max_trades: int = 3) -> List[str]:
        """自动进行交易"""
        if not hasattr(self, 'economy') or self.economy is None:
            return ["未与经济系统集成，无法交易"]
        ...
        for _ in range(max_trades):
            ...
            # 寻找最佳交易伙伴
            partner = self.find_best_trade_partner(agents)
            if not partner:
                break
            
            # 选择交易物品
            if random.random() < 0.5 and len(self.preferred_items) > 0:
                item_type = random.choice(self.preferred_items)
            else:
                item_type = random.choice(list(ItemType))
            
            # 确定数量和价格
            quantity = random.randint(1, min(5, self.get_trading_capacity()))
            market_price = self.economy.market.get_price(item_type)
            price = market_price * random.uniform(0.9, 1.1)  # 允许10%的价格浮动
            
            # 评估交易机会
            should_trade, reason = self.evaluate_trade_opportunity(item_type, quantity, price)
            if should_trade:
                # 决定买还是卖
                if self.hunger > 70 or self.thirst > 70 or self.health < 50:
                    # 需求高时优先买入
                    success, message = self.execute_trade_with(partner, item_type, quantity, price, True)
                else:
                    # 需求低时考虑卖出
                    if random.random() < 0.3:  # 30%概率主动出售
                        success, message = self.execute_trade_with(partner, item_type, quantity, price, False)
                    else:
                        continue
                
                if success:
                    trade_logs.append(f"交易成功: {message} (价格: {price:.2f})")
                    trades_made += 1
                else:
                    trade_logs.append(f"交易失败: {message}")
